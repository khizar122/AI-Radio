    ...
    if (this.b == b) {
      if (this.c) {
        try {
          ...
          console.error("Failed to play from inpoint:", e);
        }
      } else {
        try {
          this.e._resume();
          this.e._setCurrentTime(formatMillisecondsToSeconds(this.b.element[0].getAttribute('data-inpoint')));
        } catch (e) {
				console.error("CrbUI_AudioButton error resuming and seeking Flash player:", e);
        }
      }
    } else {
			this.stop();
