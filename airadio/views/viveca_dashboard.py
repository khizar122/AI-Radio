...
class LibraryView(View, JSONResponseMixin):
    ...
    def get(self, request, *args, **kwargs):
        ...
        libraries = get_page_paginator(libraries, per_page, page_number)

        if is_ajax(request):
            library_search = request.GET.get("library_q")
            station_search = request.GET.get("station_q")
            scroll = request.GET.get("scroll") == "true" or False
            ...
            if station_id:
                ...
                results = (
                    station.station_library.filter(
                        Q(library__title__icontains=station_search)
                        | Q(library__artistname__icontains=station_search)
                    )
                    if station_search
                ...
                if sort and sort in ["title", "artistname"]:
                    order_by = sort
                else:
                    order_by = "-id"

                if library_search:
                    query = Q(title__icontains=library_search) | Q(
                        artistname__icontains=library_search
                    )
                else:
                    query = Q()

                libraries = Library.objects.filter(query).order_by(order_by)
                libraries = get_page_paginator(libraries, per_page, page_number)
                context["html"] = render_to_string(
...
class MediaUploadView(View, JSONResponseMixin):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            visualise_audio = request.POST.get("visualise_audio", "false").lower() == "true"

            if not file or not type:
                raise ValueError("Missing required parameters")


            current_file_name = file._name
            file_name = create_storage_filename(type, current_file_name)
        ...
        try:
            ...
            if system_prompt and user_prompt:
                ...
                if not system_prompt:
                    context = "Failed to generate a voicetrack without system prompt!"
                elif not user_prompt:
                    context = "Failed to generate a voicetrack without user prompt!"
                else:
                    context = "Failed to generate a voicetrack with missing prompts!"
                status = False
...
class AiVoicetracksGenerateAudioView(View, JSONResponseMixin):
    @method_decorator(csrf_exempt)
    def dispatch(self, *args, **kwargs):
        return super(AiVoicetracksGenerateAudioView, self).dispatch(*args, **kwargs)
...
class NewsView(View, JSONResponseMixin):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            newsgenerator_endpoint = "https://copilot.radiostation.ai/api/news"
...
class SearchCoverImage(View, JSONResponseMixin):
    TMP_FILE_NAME = "tidal_auth_details.txt"

    @method_decorator(csrf_exempt)
    def dispatch(self, *args, **kwargs):
        return super(SearchCoverImage, self).dispatch(*args, **kwargs)

    def post(self, request, *args, **kwargs):
        TIDAL_API_URL = "https://openapi.tidal.com/search"
        ...
        try:
            response = requests.get(TIDAL_API_URL, params=params, headers=headers)
            response.raise_for_status()

            data = response.json()

            if "albums" in data and len(data["albums"]) > 0:
                img_url = data["albums"][0]["resource"]["imageCover"][0]["url"]
                context["status"] = True
            else:
                img_url = None
                context["status"] = False
            ...
            logger.error("Error making the Tidal request:", http_err)
            img_url = None
            context["status"] = False

        context["html"] = render_to_string(
            "viveca_dashboard/partials/cover_image_search_result.html",
            {"img_url": img_url},
        )

        return self.render_to_json_response(context, **kwargs)

    def saveAuthDetailsLocally(self, auth_details):
        """Save auth details to a temporary file.
        {
            "access_token": "aaaa",
            "expires_in": 86400,
            "token_type": "Bearer",
            "expires_ts": "current_ts + 86400",
            "scope": "",
        }
        """
        if auth_details is not None:
            jsonified = auth_details.json()
            jsonified["expires_ts"] = (
                int(datetime.now().timestamp()) + jsonified["expires_in"]
            )
            with open(self.TMP_FILE_NAME, "w") as f:
                f.write(json.dumps(jsonified))

    def getLocalAuthDetails(self):
        if os.path.exists(self.TMP_FILE_NAME):
            with open(self.TMP_FILE_NAME, "r") as f:
                return f.read()
        return None

    def getAccessToken(self):
        auth_token_object = self.getLocalAuthDetails()
        if auth_token_object:
            auth_token_json = json.loads(auth_token_object)
            current_datetime_ts = int(datetime.now().timestamp())

            if current_datetime_ts + 60 < auth_token_json["expires_ts"]:
                return auth_token_json["access_token"]

        auth_token_object = self.generateAccessToken()
        self.saveAuthDetailsLocally(auth_token_object)
        if auth_token_object is not None:
            return auth_token_object.json()["access_token"]
        return " "

    def generateAccessToken(self):
        try:
            TOKEN_URL = "https://auth.tidal.com/v1/oauth2/token"
            TIDAL_API_KEY = getattr(settings, "TIDAL_API_KEY", "")
            TIDAL_API_SECRET = getattr(settings, "TIDAL_API_SECRET", "")

            data = {
                "client_id": TIDAL_API_KEY,
                "client_secret": TIDAL_API_SECRET,
            ...
            if response.status_code == 200:
                ...
                logger.error("Failed to create new access token:", response.text)
        except Exception as exception:
            logger.error("Failed to create new access token", exception)
        return None
